...
namespace hololabCamera
{
    ...
    public class RtspServer : IDisposable
    {
        ...
        //private VideoFileReader video_source = null;
        private VideoFileSource video_source = null;
        private TestCard video_sourceT = null;
        private VideoFileReader videoSourceR = null;
        ...
        {
            if(!_RTSPServerListener.Server.Connected) _RTSPServerListener = new TcpListener(IPAddress.Any, 8554);
            ...
            try
            {
                while (!_Stopping.WaitOne(0))
                {
                    ...
                    lock (rtsp_list)
                    {
                        ...
                        //int video_channel = new_connection.video_transport_reply.Interleaved.First;
                        //new_connection.listener.BeginSendData(1, videoSourceR.Save(), new AsyncCallback(new_connection.listener.EndSendData), new object());
                    ...
                    //{
                    //    //    if(connection.video_transport_reply != null)
                    //    //    {
                    //    int video_channel = connection.video_transport_reply.Interleaved.First;
                    //    connection.listener.BeginSendData(video_channel, videoSourceR.Save(), new AsyncCallback(connection.listener.EndSendData), new object());
        ...
        void video_source_ReceivedYUVFrame(uint timestamp_ms, int width, int height, byte[] yuv_data)
        {
                ...
                { // Convert to Array to allow us to delete from rtsp_list
                  ...
                  // do not need to send a keepalive (so we check for Socket write errors)
                    ...
                    if ((connection.video_client_transport != null) &&
                        (connection.video_client_transport.LowerTransport == hololabCamera.Messages.RtspTransport.LowerTransportType.TCP))
                    {
            ...
            lock (rtsp_list)
            {
                                                                           ...
                                                                           // eg when there is Write Error
                {
                    ...
                    if (connection.video_client_transport.LowerTransport == hololabCamera.Messages.RtspTransport.LowerTransportType.TCP) connection_type = "TCP";
                    if (connection.video_client_transport.LowerTransport == hololabCamera.Messages.RtspTransport.LowerTransportType.UDP
                    ...
                    foreach (byte[] rtp_packet in rtp_packets)
                    {
                        ...
                        if (connection.video_transport_reply.LowerTransport == hololabCamera.Messages.RtspTransport.LowerTransportType.TCP)
                        {
                        ...
                        if (connection.video_transport_reply.LowerTransport == hololabCamera.Messages.RtspTransport.LowerTransportType.UDP && connection.video_transport_reply.IsMulticast == false)
                        {
                            try
                            {
        ...
        {
            public hololabCamera.RtspListener listener = null;  // The RTSP client connection
            ...
            public hololabCamera.Messages.RtspTransport video_client_transport; // Transport: string from the client to the server
            public hololabCamera.Messages.RtspTransport video_transport_reply; // Transport: reply from the server to the client
            public hololabCamera.UDPSocket video_udp_pair = null;     // Pair of UDP sockets (data and control) used when sending via UDP
